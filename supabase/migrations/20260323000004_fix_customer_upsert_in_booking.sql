-- Migration: fix_customer_upsert_in_booking
--
-- Problem: book_appointment() was updating last_visit_at = now() and
-- incrementing visit_count on every booking. This is wrong — a customer
-- has not "visited" just by creating an appointment.
--
-- Correct behaviour:
--   - book_appointment(): only insert/update identifying fields (full_name)
--   - visit_count + last_visit_at: updated exclusively by
--     trg_appointment_completed trigger when status → 'Tamamlandı'
--
-- The trigger trg_appointment_completed / on_appointment_completed() already
-- exists and is correct. Only the customer upsert inside book_appointment()
-- needed fixing.
...
BEGIN
  ...
  -- Customer upsert — identifying fields only.
  -- visit_count / last_visit_at are owned by trg_appointment_completed.
