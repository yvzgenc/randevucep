-- Migration: add 'confirm' action to manage_appointment_by_token
-- Allows customers to confirm their own appointment via the manage link.
...
BEGIN
  ...
  ELSIF p_action = 'confirm' THEN
    IF v_appt.status = 'Onaylı' THEN
      RETURN json_build_object('ok', true, 'action', 'already_confirmed',
        'customer_name', v_appt.customer_name, 'service_name', v_appt.service_name,
        'appointment_date', v_appt.appointment_date, 'appointment_time', v_appt.appointment_time,
        'business_id', v_appt.business_id);
    END IF;
    UPDATE appointments SET status = 'Onaylı' WHERE cancel_token = p_token;
